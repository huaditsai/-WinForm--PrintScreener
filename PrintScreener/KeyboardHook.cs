...
namespace PrintScreener
{
    class KeyboardHook
    {        
        private const int WH_KEYBOARD_LL = 13;// System Code: Low Level Keyboard Hook        
        private const int WM_KEYDOWN = 0x100;// System Event Code: Key Down Event        
        private const int WM_SYSKEYDOWN = 0x104;// System Event Code: System Key Down Event
        
        private static int s_KeyboardHookHandle;// Stores the handle to the Keyboard hook procedure
        
        public static bool globalControlOnly = false;//是否只由這個Global Hook抓取鍵盤事件
        
        private static event KeyEventHandler _globalKeyDown;//Private KeyDown Event, 與GlobalKeyDown配合使用
        ...
        private static void HookKeyboard()
        {
            if (m_HookHandle == 0)
            {
                KeyboardHook.s_KeyboardHookHandle = SetWindowsHookEx(WH_KEYBOARD_LL, m_KbdHookProc,Marshal.GetHINSTANCE(Assembly.GetExecutingAssembly().GetModules()[0]), 0);
        ...
        { 
            public int VirtualKeyCode;// Specifies a virtual-key code. The code must be a value in the range 1 to 254.
            ...
            public int ExtraInfo;// Specifies extra information associated with the message.
        }
